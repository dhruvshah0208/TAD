...
if t1_cc <= t2_cc
    T_cc = t2_cc + (l_DCT - t1_cc*v1) / abs(v1 - v2);
else
    T_cc = t1_cc + (l_DCT + t2_cc*v2) / abs(v1 - v2);
end
...
if t1_ac <= t2_ac
    T_ac = t2_ac + (l_DCT - t1_ac*v1) / abs(v1 - v2);
else
    T_ac = t1_ac + (l_DCT + t2_ac*v2) / abs(v1 - v2);
end
...
if t1_ca <= t2_ca
    T_ca = t2_ca + (l_TCT - t1_ca*v1) / abs(v1 - v2);
else
    T_ca = t1_ca + (l_TCT + t2_ca*v2) / abs(v1 - v2);
end
...
if t1_aa <= t2_aa
    T_aa = t2_aa + (l_DCT - t1_aa*v1) / abs(v1 - v2);
else
    T_aa = t1_aa + (l_DCT + t2_aa*v2) / abs(v1 - v2);
end
...
if (T_max_min == T_cc || T_max_min == T_ca)
    evader_command = 1; % 1 for clockwise
elseif (T_max_min == T_ac || T_max_min == T_aa)
    evader_command = -1; % -1 for anticlockwise
end

if (T_min_max == T_cc || T_min_max == T_ac)
    pursuer_command = 1; % 1 for clockwise
elseif (T_min_max == T_ca || T_min_max == T_aa)
...
y = [T , t_p , t_e , -evader_command , -pursuer_command]; % Had to reverse the convention for evader/pursuer commands
